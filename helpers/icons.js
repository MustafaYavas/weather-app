...
const getWeatherIcon = (iconName, left) => {
  if (iconName === '01d') {
    return <BsSun size={left ? 60 : 36} color="var(--second-color)" />;
  } else if (iconName === '02d') {
    return <BsCloudy size={left ? 60 : 36} color="var(--second-color)" />;
  } else if (iconName === '03d' || iconName === '04d') {
    return <BsClouds size={left ? 60 : 36} color="var(--second-color)" />;
  } else if (iconName === '09d') {
    return <BsCloudRain size={left ? 60 : 36} color="var(--second-color)" />;
  } else if (iconName === '10d') {
    return (
      <BsCloudRainHeavy size={left ? 60 : 36} color="var(--second-color)" />
    );
  } else if (iconName === '11d') {
    return (
      <RiThunderstormsLine size={left ? 60 : 36} color="var(--second-color)" />
    );
  } else if (iconName === '13d') {
    return <BsCloudSnow size={left ? 60 : 36} color="var(--second-color)" />;
  } else if (iconName === '50d') {
    return <RiMistFill size={left ? 60 : 36} color="var(--second-color)" />;
